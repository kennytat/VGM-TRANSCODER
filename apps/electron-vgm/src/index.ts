...
// let ipfsClient = create({ host: 'localhost', port: 9095 }) // http://ipfs-cluster-stackos.hjm.bid/ port: 80 localhost port:9095
...
try {
	...
	ipcMain.handle('check-conf', async (event, config) => {
		...
		let result;
		const confPath = `${app.getPath('appData')}/rclone/${config.name}.conf`;
		try {
			const s3Check = async (config) => {
				return new Promise(async (resolve) => {
					let data = `[${config.name}]\n`
					Object.getOwnPropertyNames(config).forEach((val, index, array) => {
						if (val !== 'name' && val !== 'bucket') {
							data = data + `${val} = ${config[val]}\n`
						}
					});
					await fs.writeFileSync(confPath, data);
					exec(`rclone lsd --config="${confPath}" ${config.name}:${config.bucket}`, { timeout: 5000 }, (error, stdout, stderr) => {
			...
			const ipfsCheck = async (endpoint) => {
				return new Promise(resolve => {
					exec(`curl -X POST "${endpoint}/api/v0/id"`, (error, stdout, stderr) => {
						if (error) resolve(false);
						resolve(true);
					})
				})
			...
			switch (config.name) {
				case "warehouse-vgm-local":
					warehouseConf.confPath = confPath;
					result = await s3Check(config);
					break;
				case "encrypted-vgm-local":
					encryptedConf.confPath = confPath;
					result = await s3Check(config);
					break;
				case "ipfs-vgm-local":
					ipfsGateway = config.endpoint;
					result = await ipfsCheck(config.endpoint);
					break;
				default:
					break;
			}
	...
	ipcMain.handle('start-convert', async (event, argInPath, argOutPath, fileOnly, pItem) => {
		...
		//   let fileInfo: FileInfo = { pid: '', md5: '', name: '', size: 0, duration: '', qm: '', url: '', hash: '', isVideo: false, dblevel: 0 };
		//   let metaData: any = [];
		//   // get file Info
		...
		//   const nonVietnamese = nonAccentVietnamese(fileInfo.name);
		//   fileInfo.url = `${pItem.url}.${nonVietnamese.toLowerCase().replace(/[\W\_]/g, '-').replace(/-+-/g, "-")}`;
		//   fileInfo.md5 = `${pItem.md5}/${nonVietnamese.replace(/\s/g, '')}`;
		//   const outPath = `${argOutPath}/${nonVietnamese.replace(/\s/g, '')}`;
		//   fileInfo.isVideo = pItem.isVideo;
		//   fileInfo.pid = pItem.id;
		//   fileInfo.dblevel = pItem.dblevel + 1;
