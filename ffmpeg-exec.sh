...
i=0
out=""
# IFS is for seperating each inputpath by \n (newline) or by ' ' (whitespace and quotation)
OIFS="$IFS"
IFS=$','
inPath="`find $arg1 -regextype posix-extended -regex '.*.(mkv|mp4)'`"
IFS=$'\n'
for f in $inPath; do
# Rename outputpath if contain whitespace to underscore
rpws="${f// /_}"
filepath="${rpws%.*}"
...
ffprobe -v quiet -show_entries format=filename,duration,size -of default=noprint_wrappers=1 -print_format json "${f}" >> $outPath/.create-db-info.txt && \
...
((i++)); done
rm -rf $outPath/.ffprobe-frame.txt $outPath/.ffmpeg-progress.txt
IFS=$SAVEIFS 
