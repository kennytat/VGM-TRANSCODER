...
[v4]scale=w=640:h=360:force_original_aspect_ratio=decrease,pad=640:360:(ow-iw)/2:(oh-ih)/2[v4out]" \
		-map "[v1out]" -c:v h264 -preset slow -rc:v vbr -crf 28 -b:v:0 "$rate1080"K -maxrate:v:0 "$rate1080"K -bufsize:v:0 "$buf1080"K -g 48 -sc_threshold 0 -keyint_min 48 \
		-map "[v2out]" -c:v h264 -preset slow -rc:v vbr -crf 28 -b:v:1 "$rate720"K -maxrate:v:1 "$rate720"K -bufsize:v:1 "$buf720"K -g 48 -sc_threshold 0 -keyint_min 48 \
		-map "[v3out]" -c:v h264 -preset slow -rc:v vbr -crf 28 -b:v:2 "$rate480"K -maxrate:v:2 "$rate480"K -bufsize:v:2 "$buf480"K -g 48 -sc_threshold 0 -keyint_min 48 \
		-map "[v4out]" -c:v h264 -preset slow -rc:v vbr -crf 28 -b:v:3 "$rate360"K -maxrate:v:3 "$rate360"K -bufsize:v:3 "$buf360"K -g 48 -sc_threshold 0 -keyint_min 48 \
		-map "0:a?" -c:a:0 aac -b:a:0 192k -ac 2 \
		-map "0:a?" -c:a:1 aac -b:a:1 128k -ac 2 \
		-map "0:a?" -c:a:2 aac -b:a:2 96k -ac 2 \
...
ffmpegThumbnail() {
	...
	ffmpeg -v quiet -y -ss 00:00:10 ${inputOption} -i "${inPath}" \
		...
		-vf ${scale}=1280:720,${outputOption} "$outPath"/720/%01d.webp \
		-vf ${scale}=854:480,${outputOption} "$outPath"/480/%01d.webp \
		-vf ${scale}=640:360,${outputOption} "$outPath"/360/%01d.webp \
		-vf ${scale}=426:240,${outputOption} "$outPath"/240/%01d.webp
}
